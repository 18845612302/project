  ...
  <div>
    ...
    >
      <el-form-item prop="name">
        <el-input v-model="search.name" placeholder="根据姓名查询"></el-input>
      </el-form-item>
      ...
      <el-form-item prop="manager">
        <el-input v-model="search.manager" placeholder="学管"></el-input>
      </el-form-item>
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" @click="searchData"
          >查询</el-button
        >
        <el-button type="primary" icon="el-icon-edit" @click="handelAdd">新增</el-button>
    ...
    <el-table :data="students" style="width: 100%">
        ...
        <template slot-scope="props">
          ...
          >
            ...
            <el-form-item label="授课教师">
              <span>{{
                props.row.teacher_id | teachersOrMangerFilter(teacherOptions)
              }}</span>
            </el-form-item>
            <el-form-item label="学管">
              <span>{{
                props.row.manager_id | teachersOrMangerFilter(managerOptions)
              }}</span>
            </el-form-item>
      ...
      <el-table-column label="所在班级" prop="grade"> </el-table-column>
      <el-table-column label="授课教师" prop="teacher_id">
        <template slot-scope="scope">
          <span>{{
            scope.row.teacher_id | teachersOrMangerFilter(teacherOptions)
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="学管" prop="manager_id">
        <template slot-scope="scope">
          <span>{{
            scope.row.manager_id | teachersOrMangerFilter(managerOptions)
          }}</span>
        </template>
...
export default {
  ...
  filters: {
    //过滤器 转换表格当中数据展示内容
    teachersOrMangerFilter(_id, options) {
...
this.$refs[formName].resetFields()
    ...
    setSrcList(imgList) {
      return imgList.map((item) => this.base + "/upload/" + item);
    },
      ...
      userApi.getUserAll().then((response) => {
        ...
        if (res.status == 0) {
          ...
          this.userAll.map((item) => {
            if (item.role_id == "61d7089787192f79dea2f5d2") {
              this.teacherOptions.push(item);
            } else if (item.role_id == "61d70a2087192f79dea2f5d8") {
